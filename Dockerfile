...
&& autoreconf -i --force \
&& ./build.sh \
&& ./configure --prefix=$MA --enable-optimise --enable-extff --with-jpeg=$MA --with-tiff=$MA --with-nifti=/usr \
&& make \
&& make install
...
&& autoreconf -i --force \
&& ./build.sh \
&& ./configure --enable-optimise --with-wlz-incl=/opt/MouseAtlas/include/ \
--with-wlz-lib=/opt/MouseAtlas/lib/ --with-fcgi-lib=/opt/MouseAtlas/lib/ --with-fcgi-incl=/opt/MouseAtlas/include/ \
--with-nifti-incl=/usr/local/include/ --with-nifti-lib=/usr/local/lib/ \
&& make
